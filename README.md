...
A full-stack, production-ready system that allows users to query a MongoDB healthcare database using natural language, powered by Groq.
...
3. Copy `.env.example` to `.env` and fill in your credentials:
   ...
   cp .env.example .env
   ```
   *Make sure to add your `GROQ_API_KEY` and set your `MONGO_URI`.*
...
## Features
- **Natural Language Query**: Type queries like "Show patients from India with Diabetes" and Groq will construct the MongoDB query.
